...
class LevelEndScreen(AbstractScreen):
    ...
    def layout_elements(self):
        ...
        self.label = UILabel(relative_rect=label_rect, text=f"Рівень {self.level} пройдено!", manager=self.manager)
        self.elements.append(self.label)

        next_button_rect = Rect((self.window_surface.get_width() // 2 - 100, 120), (200, 50))
        self.next_button = UIButton(relative_rect=next_button_rect, text="Наступний рівень", manager=self.manager)
        self.elements.append(self.next_button)

        retry_button_rect = Rect((self.window_surface.get_width() // 2 - 100, 190), (200, 50))
        self.retry_button = UIButton(relative_rect=retry_button_rect, text="Пройти ще раз", manager=self.manager)
        self.elements.append(self.retry_button)

        menu_button_rect = Rect((self.window_surface.get_width() // 2 - 100, 260), (200, 50))
        self.menu_button = UIButton(relative_rect=menu_button_rect, text="Вийти в меню", manager=self.manager)
