...
class AbstractBonusObject(AbstractMovableObject):
    ...
    def __init__(self, x_position: float,
                 y_position: float,
                 height: float, width: float, color: pygame.Color,
                 is_visible, speed: float, move_direction: list,
                 radius: float, is_active: bool):
    ...
    def move_to(self, x: float, y: float):
        # встановлюємо напрямок руху а потім рухаємо
        # в тому напрямку мяч бонусу змінюючи координати х та у
    ...
    def update_position(self):
        """
        Оновлює позицію м'яча згідно з
        поточним напрямком руху та швидкістю.
        """
        self.x_position += self.speed * self.move_direction[0]
        self.y_position += self.speed * self.move_direction[1]

    def render(self, screen):
        pygame.draw.circle(screen, self.color, (int(self.x_position),
                           int(self.y_position)), int(self.radius))
