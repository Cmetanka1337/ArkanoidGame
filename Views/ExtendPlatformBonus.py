...
class ExtendPlatformBonus(AbstractBonusObject):
    ...
    def activate(self, target: "UserPlateObject"):
        self.start_time = pygame.time.get_ticks()  # встановлюємо час активації один раз
        self.is_active = True
    ...
    def update(self, target: "UserPlateObject"):
        if self.is_active and self.start_time is not None:
            ...
            elapsed_time = (current_time - self.start_time) / 1000.0  # переведення в секунди
        ...
        if not self.is_active and self.is_visible and self.move_direction[1] > 0:
            if self.y_position + self.radius >= user_plate.rect.y:
                if user_plate.rect.x <= self.x_position <= user_plate.rect.x + user_plate.rect.width:
                    ...
                    self.is_active = True
                    # Передаємо платформу, а не GameScreen, оскільки платформа має метод resize
